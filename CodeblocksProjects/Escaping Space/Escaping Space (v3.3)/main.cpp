...
>-
>- [version 0.3.3]
>- Thanks to Andrew Do for helping with play-testing the game
>- Thanks to Cole Smalldon for helping play-test
>- Thanks to John Rosenburger for helping play-test
>- Thanks to Julia Dolansky-Overland for helping play-test
>- Thanks to Mathew Gotceitas for helping test the game
>- Thanks to Owen Stephens for helping play-test
>- Thanks to Raghav Srinivasan for helping play-test the game
>- Thanks to Vedaant Srivastava for the error trapping system and play-testing
>-
>-Thanks to Thomas Maloley for teaching me how to program with C++
>-
>- Thanks 343 and Bungie for creating HALO
>- Thanks Bethesda and Obsidian for creating Fallout
>- Thanks Valve for creating Portal and Portal 2
>-
>- [TO DO]
>- Debugging
>-
...
void scavenging( string, string, int, int [], int, string [], int, bool [], int, int [] ); //Menu that allows scavenging for suplies
...
int habRoom( string, string, int, int[], int, int[], int, bool, int, string[], bool[], bool[], bool[], bool[] ); //What the player experiences when waking up. Requires a player name, location, authority, playerStats, foundMaterials, knowing if player is starting, scenario, playerItemsStrings, knowing if the player is scavenging, knowing if power is restored and knowing if the player wants to play again
int airLockRoom( string, string, int, int[], int, int[], int, bool, int, string[], bool[] ); //The airlock room requires a player name, location, authority, playerStats, foundMaterials, knowing if player is starting, scenario, playerItemsStrings and knowing if the player is scavenging
int cafRoom( string, string, int, int[], int, int[], int, bool, int, string[], bool[] ); //The cafeteria room  requires a player name, location, authority, playerStats, foundMaterials, knowing if player is starting, scenario, playerItemsStrings and knowing if the player is scavenging
int solarCellA( string, string, int, int[], int, int[], int, bool, int, string[], bool[], bool[] ); //The solar cell A maintenance room requires a player name, location, authority, playerStats, foundMaterials, knowing if player is starting, scenario, playerItemsStrings and knowing if the player is scavenging
int medRoom( string, string, int, int[], int, int[], int, bool, int, string[], bool[] ); //The solar cell b maintenance room  requires a player name, location, authority, playerStats, foundMaterials, knowing if player is starting, scenario, playerItemsStrings and knowing if the player is scavenging
int solarCellB( string, string, int, int[], int, int[], int, bool, int, string[], bool[], bool[]); //The solar cell A maintenance room requires a player name, location, authority, playerStats, foundMaterials, knowing if player is starting, scenario, playerItemsStrings, knowing if the player is scavenging and knowing if the player wants to play again
int reactorRoom( string, string, int, int[], int, int[], int, bool, int, string[], bool[]); //The reactor room requires a player name, location, authority, playerStats, foundMaterials, knowing if player is starting, scenario, playerItemsStrings and knowing if the player is scavenging
int commandDeck( string, string, int, int[], int, int[], int, bool, int, string[], bool[]); //The command deck requires a player name, location, authority, playerStats, foundMaterials, knowing if player is starting, scenario, playerItemsStrings and knowing if the player is scavenging
int suppliesRoom( string, string, int, int[], int, int[], int, bool, int, string[], bool[], bool[]); //The spply room requires a player name, location, authority, playerStats, foundMaterials, knowing if player is starting, scenario, playerItemsStrings, knowing if the player is scavenging and knowing if the player wants to play again
int commsRoom( string, string, int, int[], int, int[], int, bool, int, string[], bool[]); //The comms room requires a player name, location, authority, playerStats, foundMaterials, knowing if player is starting, scenario, playerItemsStrings and knowing if the player is scavenging
...
int main()
{
    ...
    bool scavenged [10]; //A array that allows the code to know where has already been scavenged
    ...
    do
    {
        ...
        playerItemsStrings [6] = "ONI keycard";
        ...
        playerChoice = habRoom( playerName, locationSerialCode, adminPower, playerStats, 6, foundMaterials, 10, starting, scenario, playerItemsStrings, scavenged, recoveredPower, wokeUp, playAgain );
        ...
        while( playingGame == true )
        {
            ...
            if(playerChoice == 2)
            {
                ...
                playerChoice = medRoom( playerName, locationSerialCode, adminPower, playerStats, 6, foundMaterials, 10, starting, scenario, playerItemsStrings, scavenged);
            }
            if(playerChoice == 3)
            {
                locationSerialCode = "CAF 1D9"; //Setting location to the cafeteria
                playerChoice = cafRoom( playerName, locationSerialCode, adminPower, playerStats, 6, foundMaterials, 10, starting, scenario, playerItemsStrings, scavenged );
            }
            if(playerChoice == 4)
            {
                locationSerialCode = "SOL B02"; //Setting location to the solar cell B
                playerChoice = solarCellB( playerName, locationSerialCode, adminPower, playerStats, 6, foundMaterials, 10, starting, scenario, playerItemsStrings, scavenged, playAgain );
            }
            if(playerChoice == 5)
            {
                locationSerialCode = "SOL A01"; //Setting location to the solar cell A
                playerChoice = solarCellA( playerName, locationSerialCode, adminPower, playerStats, 6, foundMaterials, 10, starting, scenario, playerItemsStrings, scavenged, recoveredPower );
            }
            if(playerChoice == 6)
            {
                locationSerialCode = "ALO 557"; //Setting location to the airlock
                playerChoice = airLockRoom( playerName, locationSerialCode, adminPower, playerStats, 6, foundMaterials, 10, starting, scenario, playerItemsStrings, scavenged );
            }
            if(playerChoice == 7)
            {
                locationSerialCode = "RTR D8A"; //Setting location to the reactor room
                playerChoice = reactorRoom( playerName, locationSerialCode, adminPower, playerStats, 6, foundMaterials, 10, starting, scenario, playerItemsStrings, scavenged );
            }
            if(playerChoice == 8)
            {
                locationSerialCode = "CND 3C9"; //Setting location to the command deck
                playerChoice = commandDeck( playerName, locationSerialCode, adminPower, playerStats, 6, foundMaterials, 10, starting, scenario, playerItemsStrings, scavenged );
            }
            if(playerChoice == 9)
            {
                locationSerialCode = "CMS 2SL"; //Setting location to the communication room
                playerChoice = commsRoom( playerName, locationSerialCode, adminPower, playerStats, 6, foundMaterials, 10, starting, scenario, playerItemsStrings, scavenged );

            }
            if(playerChoice == 10)
            {
                locationSerialCode = "SPL 9J2"; //Setting location to the suplies room
                playerChoice = suppliesRoom( playerName, locationSerialCode, adminPower, playerStats, 6, foundMaterials, 10, starting, scenario, playerItemsStrings, scavenged, playAgain );
            }
...
void escapePod( string playerName, string location, int adminAuth )
{
    ...
    cout << ">- [version 0.3.2]" << endl;
    cout << endl;
    cout << "Project by: Max Wong" << endl;
    cout << endl;
    cout << "Thanks to:" << endl;
    cout << "               >- Andrew Do for helping with play-testing the game" << endl;
    cout << "               >- Cole Smalldon for helping test the game" << endl;
    cout << "               >- John Rosenburger for helping play-test" << endl;
    cout << "               >- Julia Dolansky-Overland for play-testing" << endl;
    cout << "               >- Matthew Gotceitas for helping test" << endl;
    cout << "               >- Owen Stephens for play-testing" << endl;
    ...
    cout << endl;
    cout << "               >- Thomas Maloley for teaching me how to use C++" << endl;
    cout << endl;
    ...
    getch();
    cout << "[Please press any key again to continue]" << endl;
    getch();
    cout << "[ Ooh, cool ending credits, you better not have skipped over this part :/ ]" << endl;
    getch();
    system("CLS");
    return;
}
//communications room
int commsRoom( string playerName, string location, int adminAuth, int playerStats[], int statsSize, int foundMaterials[], int materialsSize, bool starting, int scenario, string itemNames[], bool scavenged[] )
{
    ...
    while( usingRoom == true )
    {
        ...
        cout << "//Hopefully any ship in the general area will pick up your distress signal" << endl;
        cout << endl;
        getch();
        ...
        //If player chooses to go to scavenging
        if( inputValue == 1 )
        {
            system("CLS");
            scavenging( playerName, location, adminAuth, foundMaterials, 10, itemNames, 10, scavenged, 10, playerStats );
        }
...
//supply room
int suppliesRoom( string playerName, string location, int adminAuth, int playerStats[], int statsSize, int foundMaterials[], int materialsSize, bool starting, int scenario, string itemNames[], bool scavenged[], bool playAgain[] )
{
    bool usingRoom = true; //This bool holds true if the player is using the room. Turn false to end the loop
    ...
    while( usingRoom == true )
    {
        ...
        cout << "//This is a great chance to scavenge" << endl;
        cout << endl;
...
//Command Deck
int commandDeck( string playerName, string location, int adminAuth, int playerStats[], int statsSize, int foundMaterials[], int materialsSize, bool starting, int scenario, string itemNames[], bool scavenged[] )
{
    bool usingRoom = true; //This bool holds true if the player is using the room. Turn false to end the loop
    ...
    while( usingRoom == true )
    {
        ...
        cout << "//Sitting in the middle of the room is a big chair and at the very front of the room stands two large glass windows" << endl;
        cout << endl;
        ...
        if( inputValue == 4 )
        {
            ...
            if( foundMaterials[5] >= 1 && foundMaterials[6] >= 1 && foundMaterials[9] >= 1 )
            {
                ...
                cout << "//Thankfully this door has a tiny manual override switch but it still requires a keycard to activate" << endl;
                getch();
                cout << endl;
...
//Reactor room
int reactorRoom( string playerName, string location, int adminAuth, int playerStats[], int statsSize, int foundMaterials[], int materialsSize, bool starting, int scenario, string itemNames[], bool scavenged[] )
{
    bool usingRoom = true; //This bool holds true if the player is using the room. Turn false to end the loop
    ...
    while( usingRoom == true )
    {
        ...
        cout << endl;
        cout << "//You hit a large red button on the side and immediately a strong looking foam emerges and covers the hole" << endl;
        ...
        cout << "//What would you like to do?" << endl;
        cout << endl;
        cout << "[1]Look around for any scavengable supplies" << endl;
...
int solarCellB( string playerName, string location, int adminAuth, int playerStats[], int statsSize, int foundMaterials[], int materialsSize, bool starting, int scenario, string itemNames[], bool scavenged[], bool playAgain[] )
{
    bool usingRoom = true; //This bool holds true if the player is using the room. Turn false to end the loop
    bool isDead = false; //If player is dead, this is true
    ...
    while( usingRoom == true )
    {
        ...
        cout << "//once again a heavily damaged wall is covered in burn marks " << endl;
        cout << endl;
        ...
        cout << "//What would you like to do?" << endl;
        cout << endl;
        cout << "[1]Look around for any scavengeable supplies" << endl;
...
//Medical room
int medRoom( string playerName, string location, int adminAuth, int playerStats[], int statsSize, int foundMaterials[], int materialsSize, bool starting, int scenario, string itemNames[], bool scavenged[] )
{
    bool usingRoom = true; //This bool holds true if the player is using the room. Turn false to end the loop
    ...
    while( usingRoom == true )
    {
        ...
        cout << "//This room is filled with many cabinets, definitely a good chance to scavenge some supplies" << endl;
        cout << endl;
...
int solarCellA( string playerName, string location, int adminAuth, int playerStats[], int statsSize, int foundMaterials[], int materialsSize, bool starting, int scenario, string itemNames[], bool scavenged[], bool recoveredPower[] )
{
    bool usingRoom = true; //This bool holds true if the player is using the room. Turn false to end the loop
...
//If player has not fixed the solar cell yet
        ...
        cout << "//What would you like to do?" << endl;
        cout << endl;
        cout << "[1]Scavenge the room for any supplies" << endl;
        ...
        if( inputValue == 3)
        {
            if( recoveredPower == false )
            {
                ...
                cout << "//Next you observe that someone has violently pulled the power connection plugs out" << endl;
                cout << "//The outlet is damaged so any permanent attempts to restore power from this cell will require these plugs to be permanently plugged in" << endl;
                getch();
            }
            ...
            if( foundMaterials[5] >= 1 && foundMaterials[1] >= 1 && foundMaterials[0] >= 1 && recoveredPower[0] == false)
            {
                ...
                cout << "//For several minutes, no explosions or fires occur so you feel reasonably safe" << endl;
                cout << endl;
...
//Cafeteria room
int cafRoom( string playerName, string location, int adminAuth, int playerStats[], int statsSize, int foundMaterials[], int materialsSize, bool starting, int scenario, string itemNames[], bool scavenged[] )
{
    bool usingRoom = true; //This bool holds true if the player is using the room. Turn false to end the loop
    ...
    while( usingRoom == true )
    {
        ...
        cout << "//What would you like to do?" << endl;
        cout << endl;
        cout << "[1]Scavenge the body and the room for supplies" << endl;
        cout << "[2]Return to the HAB" << endl;
        cout << "[3]Continue forwards" << endl;
        cin >> inputAnything;
        //Call function for all menu inputs
...
//Air lock room
int airLockRoom( string playerName, string location, int adminAuth, int playerStats[], int statsSize, int foundMaterials[], int materialsSize, bool starting, int scenario, string itemNames[], bool scavenged[] )
{
    bool usingRoom = true; //This bool holds true if the player is using the room. Turn false to end the loop
    ...
    while( usingRoom == true )
    {
        ...
        cout << "//Each shelf is sealed with scraps of materials that no doubt were used for excursions outside the station in the past" << endl;
        cout << "//Now they are a great chance for you to find potentialy useful supplies" << endl;
        cout << endl;
        cout << "//To the side stands a terminal" << endl;
        cout << endl;
        cout << "//What would you like to do?" << endl;
        cout << endl;
        cout << "[1]Scavenge the shelves" << endl;
        cout << "[2]Return to the HAB" << endl;
...
int habRoom( string playerName, string location, int adminAuth, int playerStats[], int statsSize, int foundMaterials[], int materialsSize, bool starting, int scenario, string itemNames[], bool scavenged[], bool recoveredPower[], bool wokeUp[], bool playAgain[] )
{
    std::string inputAnything = "";// Variable where you can input anything
    ...
    while( usingRoom == true )
    {
        ...
        cout << "[5]All around you lies scraps and shelves that you could possibly scavenge something from" << endl;
        storyOver = true;
        //Ask what used would like to input
        getch;
        cout << endl;
        cout << "//What would you like to do? Choose a number or a menu prompt to continue" << endl;
        cout << endl;
        cin >> inputAnything;
        //Call function for all menu inputs
        playerChoice = checkInput( inputAnything, playerName, location, adminAuth, starting, playerStats, foundMaterials, scenario, playerChoice );
...
void scavenging  (string playerName, string location, int authority, int foundItems[], int itemsSize, string itemNames[], int namesSize, bool scavenged[], int scavengedSize, int playerStats[] )
{
    ...
    //If location (HAB) is right but player has already scavenged in this location
    if( location == "HAB 4K2" && scavenged[0] == true )
    {
        cout << endl;
        cout << ">- sorry, you have already scavenged" << endl;
        cout << endl;
    }
    //If location (HAB) is right but player has not scavenged in this location yet:
    if( location == "HAB 4K2" && scavenged[0] == false )
    {
        for(int i = 0; i <= 10; i++)
        {
    ...
    //If location (Airlock) is right but player has already scavenged in this location
    if( location == "ALO 557" && scavenged[5] == true )
    {
        cout << endl;
        cout << ">- sorry, you have already scavenged" << endl;
        cout << endl;
    }
    //If location (Airlock) is right but player has not scavenged in this location yet:
    if( location == "ALO 557" && scavenged[5] == false )
    {
        for(int i = 0; i <= 10; i++)
        {
            //Generate a random nummber
    ...
    //If location (Cafeteria) is right but player has already scavenged in this location
    if( location == "CAF 1D9" && scavenged[2] == true )
    {
        cout << endl;
        cout << ">- sorry, you have already scavenged" << endl;
        cout << endl;
    }
    //If location (Cafeteria) is right but player has not scavenged in this location yet:
    if( location == "CAF 1D9" && scavenged[2] == false )
    {
        for(int i = 0; i <= 10; i++)
        {
            //Generate a random nummber
    ...
    //If location (solar cell A) is right but player has already scavenged in this location
    if( location == "SOL A01" && scavenged[4] == true )
    {
        cout << endl;
        cout << ">- sorry, you have already scavenged" << endl;
        cout << endl;
    }
    //If location (Solar Cell A) is right but player has not scavenged in this location yet:
    if( location == "SOL A01" && scavenged[4] == false )
    {
        for(int i = 0; i <= 10; i++)
        {
            //Generate a random nummber
    ...
    //If location (Medbay) is right but player has already scavenged in this location
    if( location == "MED 89S" && scavenged[1] == true )
    {
        cout << endl;
        cout << ">- sorry, you have already scavenged" << endl;
        cout << endl;
    }
    //If location (Medbay) is right but player has not scavenged in this location yet:
    if( location == "MED 89S" && scavenged[1] == false )
    {
        for(int i = 0; i <= 10; i++)
        {
            //Generate a random nummber
    ...
    //If location (solar cell B) is right but player has already scavenged in this location
    if( location == "SOL B02" && scavenged[3] == true )
    {
        cout << endl;
        cout << ">- sorry, you have already scavenged" << endl;
        cout << endl;
    }
    //If location (solar cell B) is right but player has not scavenged in this location yet:
    if( location == "SOL B02" && scavenged[3] == false )
    {
        for(int i = 0; i <= 10; i++)
        {
            //Generate a random nummber
    ...
    //If location (Reactor) is right but player has already scavenged in this location
    if( location == "RTR D8A" && scavenged[6] == true )
    {
        cout << endl;
        cout << ">- sorry, you have already scavenged" << endl;
        cout << endl;
    }
    //If location (Reactor) is right but player has not scavenged in this location yet:
    if( location == "RTR D8A" && scavenged[6] == false )
    {
        for(int i = 0; i <= 10; i++)
        {
            //Generate a random nummber
    ...
    //If location (Command deck) is right but player has already scavenged in this location
    if( location == "CND 3C9" && scavenged[7] == true )
    {
        cout << endl;
        cout << ">- sorry, you have already scavenged" << endl;
        cout << endl;
    }
    //If location (Command deck) is right but player has not scavenged in this location yet:
    if( location == "CND 3C9" && scavenged[7] == false )
    {
        for(int i = 0; i <= 10; i++)
        {
            //Generate a random nummber
    ...
    //If location (Medbay) is right but player has already scavenged in this location
    if( location == "SPL 9J2" && scavenged[9] == true )
    {
        cout << endl;
        cout << ">- sorry, you have already scavenged" << endl;
        cout << endl;
    ...
    //If location (Medbay) is right but player has not scavenged in this location yet:
    if( location == "SPL 9J2" && scavenged[9] == false )
    {
        for(int i = 0; i <= 10; i++)
        {
            //Generate a random nummber
    ...
    //If location (Command deck) is right but player has already scavenged in this location
    if( location == "CMS 2SL" && scavenged[8] == true )
    {
        cout << endl;
        cout << ">- sorry, you have already scavenged" << endl;
        cout << endl;
    ...
    //If location (Command deck) is right but player has not scavenged in this location yet:
    if( location == "CMS 2SL" && scavenged[8] == false )
    {
        for(int i = 0; i <= 10; i++)
        {
            //Generate a random nummber
...
void experienceMenu ( int playerStats[], int size, string playerName, string serialCode, int auth, bool starting)
{
    ...
    do
    {
        ...
        if(playerStats[3] > 0)
        {
            ...
            cout << ">- [2] Resilience (Increase health)" << endl;
            cout << ">- [3] Intelligence (Increase XP)" << endl;
        ...
        if(playerStats[3] < 1)
        {
            do
            {
                ...
                cout << endl;
                cout << ">- do you confirm these stats? /y/ for yes, /n/ for no" << endl;
                cin >> confirmInput;
...
//If input is invalid, print an error message
            ...
            if( inputValue != 9 && enterAnything != "a" && enterAnything != "A" && playerItems[inputValue - 1] != 0 )
            {
                cout << endl;
            ...
            if( inputValue == 9 && playerItems[8] == 1)
            {
                ...
                cout << endl;
                cout << "   Anyways, the ONI code to the escape pods is" << endl;
                cout << endl;
...
bool hackTerminal(  string playerName, string locationCode, int authority, int playerItems[], int size )
{
    ...
    cout << ">- HINT: Guess the developers birthday (This is not the same password on the note)" << endl;
    getch();
    ...
    while(gettingInput == true)
    {
        ...
        if( locationCode == "CND 3C9" && playerItems[6] >= 1 )
        {
            ...
            cout << endl;
            cout << ">- Enter the password below [press A to quit]" << endl;
            cout << ">- HINT: Try looking for a note in the items list and using it" << endl;
            cin >> enterAnything;
            //if player chooses to quit
            if( enterAnything == "a" || enterAnything == "A" )
            {
...
string mainMenu( bool menuLoad, bool askName)
{
    ...
    if(menuLoad == true)
    {
        ...
        cout << ">- [Keep Pressing Any Key To Continue]" << endl;
        getch();
        cout << endl;
        ...
        cout << ">- HAB -------------------- Active at 38% efficiency" << endl;
        cout << endl;

        getch();
        cout << ">- Maintain emergency oxygen supply at 20.5%" << endl;
        cout << endl;
        ...
        if(askName == true)
        {
            ...
            cout << ">- [**Note, you will be referred as this ID for the rest of the game]" << endl;
            cout << endl;
            cin >> playerName;
...
bool takeDamage( int playerStats[], int size, string location, bool playAgain[] )
{
    ...
    {
        cout << endl;
        cout << "//As you finish fiddling with the box, all of a sudden, the box starts sparking, than explodes" << endl;
        cout << endl;
