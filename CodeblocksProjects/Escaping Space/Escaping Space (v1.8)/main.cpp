...
void habRoom( string, string, int, int[], int, int[], int, bool, int, string[], bool[]); //What the player experiences when waking up. Requires a player name, location and authority
...
int main()
{
    ...
    habRoom( playerName, locationSerialCode, adminPower, playerStats, 6, foundMaterials, 10, starting, scenario, playerItemsStrings, scavanged);

    return 0;
}

//First part of the game when you wake up
void habRoom( string playerName, string location, int adminAuth, int playerStats[], int statsSize, int foundMaterials[], int materialsSize, bool starting, int scenario, string itemNames[], bool scavanged[])
{
    ...
    do
    {
        ...
        if( inputValue == 4)
        {
            ...
            if( foundMaterials[1] >= 1 && foundMaterials[4] >= 1 && foundMaterials[9] >= 1 /* && future variable for the power coming from the repaired solar cell*/){
                ...
                cout << endl;
                cout << "//With a hiss from re-pressurization, the safety disengages and the reactor door swings open" << endl;
                cout << "//You make a note that the safety system is buggy and will most likely require a system reset to pass through again" << endl;
                cout << endl;
                getch();
                system("CLS");
            }
            ...
            if( foundMaterials[1] < 1 && foundMaterials[4] < 1 && foundMaterials[9] < 1 /* && future variable for the power coming from the repaired solar cell*/){
                ...
                cout << endl;
                cout << "//You realize that behind this sealed door may hold a reactor leak or a depressurized room. You must find a space suit if you wish to pass through" << endl;
                cout << "//You also require some wire and a wire tweezers in order to reset the computer system" << endl;
                cout << "//Finally, you also notice that the door requires extra power in order to function" << endl;
                cout << endl;
                getch();
                system("CLS");
            }
    ...
    while( inputValue != 1 && inputValue != 2 && inputValue != 3 && inputValue != 4 && inputValue != 5 && inputAnything != "a" && inputAnything != "A" && inputAnything != "i" && inputAnything != "I" && inputAnything != "m" && inputAnything != "M" && inputAnything != "v" && inputAnything != "V" );
